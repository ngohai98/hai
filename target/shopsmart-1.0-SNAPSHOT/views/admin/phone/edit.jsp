                ...
                <div class="col-xs-12">
                        ...
                        <form id="formSubmit">
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="hang_id" name="hang_id">
                                        <c:if test="${empty model.hang_id}">
                                            <option value="">Chọn nhãn hiệu</option>
                                            <c:forEach var="item" items="${brands}">
                                                <option value="${item.hang_id}">${item.hang_ten}</option>
                                            </c:forEach>
                                        </c:if>
                                        <c:if test="${not empty model.hang_id}">
                                            <option value="">Chọn nhãn hiệu</option>
                                            <c:forEach var="item" items="${brands}">
                                                <option value="${item.hang_id}" <c:if test="${item.hang_id == model.hang_id}">selected="selected"</c:if>>
                                                        ${item.hang_ten}
                                                </option>
                                            </c:forEach>
                                        </c:if>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="file" class="form-control" id="dt_HinhAnh" name="dt_HinhAnh" value="${model.dt_HinhAnh}"/>
                                </div>
                            </div>
                            <br/>
                            <br/>
                            ...
                            <input type="hidden" value="${model.dt_id}" id="dt_id" name="dt_id"/>
                        </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    function updateNew(data) {
        $.ajax({
            ...
            dataType: 'json',
            success: function (result) {
            	window.location.href = "${PhoneURL}?type=list&id="+result.dt_id+"&page=1&maxPageItem=5&message=update_success";
            },
            error: function (error) {
            	window.location.href = "${PhoneURL}?type=list&maxPageItem=5&page=1&message=error_system";
            }
