...
<c:url var="PhoneURL" value="/admin-donhanglist" />
...
<body>
	<div class="main-content">
		<form action="<c:url value='/admin-donhanglist'/>" id="formSubmit"
			method="get">
									...
									<div class="pull-right tableTools-container">
										<div class="dt-buttons btn-overlap btn-group">
											<a flag="info"
												...
												href='<c:url value="/admin-donhanglist?type=edit"/>'> <span>
												...
												<c:forEach var="item" items="${model.listResult}">
													<tr>
														<td><input type="checkbox"
															id="checkbox_${item.dh_id}" value="${item.dh_id}"></td>
														<td>${item.dh_id}</td>
														<td>${item.user.user_ten}</td>
														<td>${item.giohang.phone.dt_Ten}</td>
														<td>${item.giohang.gh_soluongmua}</td>
														<td>${item.dh_ngaylap}</td>
														<td>${item.user.user_diachi}</td>
														<td>${item.dh_tongtien}</td>
														<c:if test="${item.dh_tinhtrang==true}">
																<td>Đã xác nhận</td>
														</c:if>
														<c:if test="${item.dh_tinhtrang==false}">
																<td>Chưa xác nhận</td>
														</c:if>
														
														
														<td><c:url var="editURL" value="/admin-donhanglist">
																<c:param name="type" value="edit" />
																<c:param name="id" value="${item.dh_id}" />
